...
async function stopScript() {
	debugLogCritical('stopScript called - script stopping', null, logCallLocation())
	...
	try {
		if (!chrome || !chrome.tabs || typeof chrome.tabs.query !== 'function') {
			debugLogError('Chrome tabs API not available in stopScript', null, logCallLocation())
			prevSearchValue = ''
			return
		}
...
async function startScript() {
	...
	if (!isExtensionContextValid()) {
		debugLogError('Extension context invalid in startScript, cannot start', null, logCallLocation())
		return false
	}
	...
	return new Promise((resolve) => {
		chrome.storage.local.get('autoApplyRunning', (result) => {
			...
			if (isRunning) {
				...
				debugLogCritical('checkAndPrepareRunState: script not running, stopping process', null, logCallLocation())
				resolve(false)
				prevSearchValue = ''
			}
		})
...
async function clickJob(listItem, companyName, jobTitle, badWordsEnabled) {
	return new Promise(async (resolve) => {
		try {
			...
			if (!isRunning) {
				debugLogCritical('clickJob: script not running, aborting job processing', null, logCallLocation())
				resolve(null)
				return
			}
			...
			if (badWordsEnabled) {
				...
				if (jobDetailsElement) {
					...
					if (badWords?.length > 0) {
						...
						if (matchedBadWord) {
							debugLogInfo(`clickJob: found bad word "${matchedBadWord}", skipping job`, null, logCallLocation())
							resolve(null)
							return
						}
					}
...
async function validateAndCloseConfirmationModal() {
	...
	if (modal) {
		...
		if (
			...
			debugLogError('Save application modal found but no buttons to close it', null, logCallLocation())
		}
	}
...
async function handleSaveApplicationModal() {
	...
	if (saveModal) {
		...
		if (
			...
			debugLog('Save application modal detected and handling', null, false, logCallLocation())
			...
			if (dismissButton) {
				debugLogError('No discard button found, using dismiss as fallback', null, logCallLocation())
				dismissButton.click()
				await addDelay(1000)
				return true
			}
			
			debugLogError('Save application modal found but no way to close it', null, logCallLocation())
		}
	}
...
async function terminateJobModel(context = document) {
	...
	if (dismissButton) {
		...
		debugLogError('terminateJobModel: no dismiss button found', null, logCallLocation())
	}
}
...
async function runApplyModel() {
	try {
		return await Promise.race([
				...
				if (Array.isArray(applyModalWait)) {
					...
					if (submitButton) {
						...
						if (saveModalAfterSubmit) {
							debugLog('Save application modal handled after submit', null, false, logCallLocation())
						}
					...
					if (nextButton || reviewButton) {
						...
						if (isError) {
							debugLogError('Error detected in form validation, terminating job modal', null, logCallLocation())
							...
							if (saveModalAfterNext) {
								debugLog('Save application modal handled after next/review', null, false, logCallLocation())
							}
			...
			new Promise((resolve) => {
				setTimeout(() => {
					debugLog('runApplyModel timeout reached (30s) - this is normal behavior', null, logCallLocation())
					resolve(null)
				}, 30000)
			})
		])
	...
	return new Promise(async (resolve) => {
		try {
			...
			if (!chrome || !chrome.runtime) {
				debugLogError('Extension context invalidated in runFindEasyApply', null, logCallLocation())
				resolve(null)
				return
			}
...
async function goToNextPage() {
	...
	try {
		...
		try {
			...
			debugLogError('goToNextPage waitForElements error', error, logCallLocation())
		}
...
async function runScript() {
	debugLogInfo('runScript STARTED', { url: window.location.href, readyState: document.readyState }, logCallLocation())
	
	try {
		await addDelay(3000)
		
		if (!isExtensionContextValid()) {
			debugLogError('runScript: extension context invalid at start, stopping', null, logCallLocation())
			return
		}
		...
		if (!scriptStarted) {
			debugLogError('runScript: failed to start script, stopping', null, logCallLocation())
			return
		}
		...
		const isRunning = await checkAndPrepareRunState()
		if (!isRunning) {
			debugLogCritical('runScript: state check failed, script not running', null, logCallLocation())
			return
		}
		
		if (!isExtensionContextValid()) {
			debugLogError('runScript: extension context invalid after state check, stopping', null, logCallLocation())
			return
		}
		...
		if (!fieldsComplete) {
			debugLogCritical('runScript: default fields not configured, opening config page', null, logCallLocation())
		...
		if (limitReached) {
			debugLogCritical('runScript: daily application limit reached, stopping', null, logCallLocation())
			const feedbackMessageElement = document.querySelector(
				'.artdeco-inline-feedback__message'
			)
		...
		debugLog(`Processing ${listItems.length} job list items`, null, false, logCallLocation())
		
		for (let i = 0; i < listItems.length; i++) {
			const listItem = listItems[i]
			
			if (i % 5 === 0 && !isExtensionContextValid()) {
				debugLogError('Extension context lost during job processing', null, logCallLocation())
				return
			}
			...
			if (saveModalBefore) {
				debugLog('Save application modal handled before job processing', null, logCallLocation())
			}
			...
			if (canClickToJob) {
				...
				if (saveModalAfter) {
					debugLog('Save application modal handled after job processing', null, false, logCallLocation())
				}
			}
		}
		...
		debugLogError('Critical error in runScript', { error: error?.message, stack: error?.stack }, logCallLocation())
		console.trace(message)
		await stopScript()
	}
...
window.addEventListener('error', function(event) {
	debugLogError('Window error detected', { message: event.error?.message, filename: event.filename, lineno: event.lineno, stack: event.error?.stack }, logCallLocation())
	...
	if (
		...
		debugLogCritical('Extension context invalidated, stopping script', null, logCallLocation())
		...
		try {
			...
			debugLogError('Error hiding modal after extension context invalidation', error, logCallLocation())
		}
	}
})
...
function stopExtensionContextMonitoring() {
	debugLogInfo('Stopping extension context monitoring', null, logCallLocation())
...
function startSaveModalMonitoring() {
	saveModalCheckInterval = setInterval(async () => {
		const saveModalHandled = await handleSaveApplicationModal()
		if (saveModalHandled) {
			debugLog('Save modal detected and handled by background monitor', logCallLocation())
		}
			...
			try {
				if (shouldRestartScript && loopRestartUrl) {
					...
					if (isJobSearchPage && hasKeywords && isStartPage) {
						debugLogInfo('Window load: conditions met, restarting script', null, logCallLocation())
				...
				debugLogError('Window load: error during processing', error, logCallLocation())
			}
		}
	)
})
...
try {
	...
	debugLogError('Error setting beforeunload listener', error, logCallLocation())
}