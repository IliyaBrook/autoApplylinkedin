  ...
  if (event.target.tagName === "BUTTON") {
    ...
    switch (buttonId) {
      ...
      case "cv-files-toggle":
        try {
          let cvAccordion = document.getElementById("cvFilesAccordion");
          const dataset = button.dataset;
          
          if (dataset.open === "true") {
            button.dataset.open = "false";
            if (cvAccordion) {
              cvAccordion.style.display = "none";
            }
          } else {
            button.dataset.open = "true";
            if (cvAccordion) {
              cvAccordion.style.display = "block";
              renderCvFiles();
            }
          }
        } catch (error) {
          console.error("Cannot toggle CV files accordion", error);
        }
        break;
      case "add-cv-name":
        addNewCvFile();
...
let cvFileIdCounter = 0;

function addNewCvFile() {
  chrome.storage.local.get(["cvFiles", "selectedCvFile"], (result) => {
    const cvFiles = result.cvFiles || [];
    const newId = `cv_${Date.now()}_${cvFileIdCounter++}`;
    
    cvFiles.push({ id: newId, name: "" });
...
function renderCvFiles() {
  const content = document.querySelector(".cv-files-content");
  if (!content) return;
  ...
  chrome.storage.local.get(["cvFiles", "selectedCvFile"], (result) => {
    ...
    if (cvFiles.length === 0) {
      const emptyMsg = document.createElement("div");
      emptyMsg.textContent = "No CV names available. Click 'Add CV Name' to create one.";
    ...
    cvFiles.forEach((fileObj) => {
      const item = document.createElement("div");
      item.className = "cv-file-item";
      item.dataset.id = fileObj.id;
      
      // Add selected class if this is the selected file
      if (fileObj.id === selectedCvFile) {
      ...
      radio.type = "radio";
      radio.name = "cvFileSelector";
      radio.value = fileObj.id;
      radio.checked = fileObj.id === selectedCvFile;
      radio.onclick = () => {
        chrome.storage.local.set({ selectedCvFile: fileObj.id }, () => {
          renderCvFiles();
        });
      };
      item.appendChild(radio);
      
      // Text input (readonly by default)
      const textInput = document.createElement("input");
      textInput.type = "text";
      textInput.value = fileObj.name;
      textInput.readOnly = true;
      textInput.dataset.id = fileObj.id;
      textInput.placeholder = "Enter CV name...";
      ...
      editBtn.onclick = () => {
        if (textInput.readOnly) {
          ...
          editBtn.className = "icon-btn save-btn";
        } else {
          // Save mode
          ...
          const isDuplicate = cvFiles.some(
            (f) => f.id !== fileObj.id && f.name === newName
          );
          if (isDuplicate) {
            alert("CV name already exists.");
            return;
          }
          
          // Update the file
          chrome.storage.local.get("cvFiles", (res) => {
            const files = res.cvFiles || [];
            const fileIndex = files.findIndex((f) => f.id === fileObj.id);
            if (fileIndex !== -1) {
              files[fileIndex].name = newName;
              chrome.storage.local.set({ cvFiles: files }, () => {
                renderCvFiles();
              });
            }
          });
      ...
      delBtn.title = "Remove";
      delBtn.onclick = () => {
        if (confirm(`Are you sure you want to delete "${fileObj.name || 'this CV'}"?`)) {
          chrome.storage.local.get(["cvFiles", "selectedCvFile"], (res) => {
            const files = res.cvFiles || [];
            const selected = res.selectedCvFile;
            const fileIndex = files.findIndex((f) => f.id === fileObj.id);
            
            if (fileIndex !== -1) {
              files.splice(fileIndex, 1);
              const updates = { cvFiles: files };
              
              // If we're deleting the selected file, clear the selection
              if (selected === fileObj.id) {
                updates.selectedCvFile = null;
              }
      ...
      buttonsGroup.appendChild(delBtn);
